...
class KategorierPage extends StatefulWidget {
  final UserManager userManager;

  KategorierPage({required this.userManager});

  @override
  _KategorierPageState createState() => _KategorierPageState();
}

class _KategorierPageState extends State<KategorierPage> {
  ...
  Future<void> loadQuestions() async {
    final String response = await rootBundle.loadString('assets/other/kategorier.txt');
    setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Kategori ${currentQuestionIndex + 1}'),
